...
object CountryCodeUtils {
    ...
    suspend fun getCountryCodes(ctx: Context): ArrayList<CountryCodeDataModel>{
        val list = ArrayList<CountryCodeDataModel>()
        try {
            val assets = ctx.assets.open("country_phone_codes.json")
            ...
            for (i in 0 until json.length()){
                ...
                val data = withContext(Dispatchers.IO){ getCountryFlag(ctx, code) }
                list.add(
    ...
    private fun getCountryFlag(ctx: Context, code: String): CountryCodeFlagDataModel?{
        try {
            val assets = ctx.assets.open("country_code_name_flag.json")
            val size = assets.available()
            val buffer = ByteArray(size)
            assets.read(buffer)
            assets.close()
            ...
            return if (!json.isNull(code)){
                val obj = json.getJSONObject(code)
                CountryCodeFlagDataModel(
                    name = obj.getString("name"),
                    code = code,
                    emoji = obj.getString("emoji"),
                    image = obj.getString("image"),
                    imageDrawable = AppCompatResources.getDrawable(ctx, getCountryFlagDrawable(code))
                )
            }else{
                null
            }
    ...
    private fun getCountryFlagDrawable(code: String): Int{
